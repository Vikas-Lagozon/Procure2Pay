...
class RequirementsChatbot(BaseAgent):
    ...
    def __init__(self):
        super().__init__(
            ...
            description=(
                "Ingest multiple requirement documents, answer questions, "
    ...
    async def _handle_list(
        ...
        if not records:
            yield self._text_event(
                ...
                "Upload a document first: /upload <file_path>"
            )
            return
    ...
    async def _handle_get(
        ...
        if not rec:
            yield self._text_event(f"No record found with ID: {record_id}")
            return
    ...
    async def _handle_update(
        ...
        if len(args) < 2:
            yield self._text_event(
                ...
                "Provide the record ID and the path to the revised document.\n"
...
""",
        ...
        update_payload = {
            ...
            "file_name":  Path(stored_path).name,   # keep stored name
        }
        ...
        # Update the session entry for this record (match by record_id)
        session_key = None
        ...
        yield Event(
            ...
            content=types.Content(
                ...
                parts=[types.Part(text=(
                    ...
                    f"You can now ask questions about the updated document."
                ))],
            ),
            actions=EventActions(
                state_delta={"documents": json.dumps(existing_docs)}
...
""",
        ...
        logger.info(f"Stored document with ID: {inserted_id}")
        ...
        yield Event(
            ...
            content=types.Content(
                ...
                parts=[types.Part(text=(
                    ...
                    f"📂 Documents loaded in this session ({len(existing_docs)}):\n"
                    f"{doc_list}\n\n"
    ...
    async def _handle_question(
        ...
        if not documents:
            ...
            if not records:
                yield self._text_event(
                    "No requirement records found in the database.\n"
                    "Upload a document first using its file path or /upload <file_path>."
                )
                return

            # Build session-compatible document dict from DB records
            for rec in records:
            ...
            # Persist into session so follow-up questions don't re-query DB
        ...
        qa_agent = LlmAgent(
            ...
            model=MODEL,
            instruction=f"""
You are an expert Requirements Analyst assistant with deep knowledge of procurement and business requirements.
...
LOOKUP & DETAIL questions
  ...
  - "Show me the vendor details for printer requirements."

FILTERING & CATEGORY questions
  - "List all requirements related to home appliances."
  - "Which requirements are for electronics items?"
  - "Show me all requirements from the IT department."

COUNTING & AGGREGATION questions
  - "How many requirements are there in total?"
  - "How many requirements are related to laptops?"
  - "How many units of laptops do we need across all requirements?"

CROSS-DOCUMENT ANALYSIS questions
  - "Which document has the highest estimated cost?"
  - "Compare the technical requirements across all documents."
  - "What is the ideal specification of all printer requirements?"

UNIT & QUANTITY questions
  - "How many units are needed for each requirement?"
  - "In which requirements do we need laptops, and how many units?"

GUIDELINES:
- Always cite WHICH document your answer comes from.
- For counting/aggregation, scan ALL documents and sum up where relevant.
...
""",
        ...
        if state_delta:
            # Yield a state update event first so session is populated for follow-ups
            yield Event(
                author=self.name,
...
UPLOAD  (file stored in REQUIREMENTS/ with a timestamp prefix)
...
NATURAL LANGUAGE Q&A  (works even without uploading in this session)
  Ask anything about your requirements in plain English, for example:
...
# EXPORTS
# ─────────────────────────────────────────────────────────────
chatbot_agent = RequirementsChatbot()
